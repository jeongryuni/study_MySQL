...
value ('A부모', '30','남자', '서울', '01053210063');

insert into tbl_parent
(name, age, gender, address, phone)
value ('B부모', '32','여자', '남양주', '01083550064');

/* 부모 중, 나이가 31세 이상인 부모의 이름 조회 */
select name, age
from tbl_parent
where age>=31;
...
/* group by를 사용하면 중복이 사라진다. */
/* from으로 tbl_order 테이블 가져와 name별로 name의 개수를 세고 total로 이름을 지정하고 조회한다.*/
select name, count(name) total from tbl_order
group by name;

/* 주문된 상품별 평균 가격 조회 */
/* from으로 tbl_order 테이블 가져와 name별로 price평균을 계산하고 average로 이름을 지정하고 조회한다.*/
select name, avg(price) average from tbl_order
group by name;

/* 주문된 상품별 평균 가격 5000원 이상인 상품 조회 */
/* from절로 가져올 때 where절에서는 집계 함수를 쓸 수 없다. select절에서만 사용해야 한다.*/
/* from으로 tbl_order 테이블 가져와 name별로 price평균을 계산하고 반올림 후 average로 이름을 지정하고 조회한다.*/
...
/*  3. name(상품), sum
    1. from으로 tbl_order의 테이블을 가져온다
    2. name별로 묶는다.
*/
select name, sum(price) total,(select sum(price) from tbl_order) total2
from tbl_order
group by name;

/*상품 중 "우"가 들어간 상품별 총 합 조회*/
/*
   from으로 tbl order에서 컬럼을 불러와
   select문에서 '우'가 들어간 name의 price를
   조회한 것의 별칭을 o라고 함/
 */
...
/*
6. select로 id, name(상품명), 가격을 tbl_order테이블에서 조회한다.
5. where 바깥 tbl_order테이블의 name이 조건에 맞는 name목록 안에 있는지 검사
(
   1. from으로 tbl_order 테이블을 가져와서
   2. group by로 name을 기준으로 잡는다.
   3. having으로 price 평균가격이 1000원 이하인 상품을 조건으로 잡는다.
   4. select로 상품을 조회하고 표이름을 average로 지정한다.
)
*/
...
/*
    ...
    2. 아래 조건문이 맞다면  tbl_user u join tbl_post p 을 조인하고  p.*, u.name조회한다.
    1. tbl_post의 id가 tbl_reply의 post_id 중에 3회 이상 나타나는 경우
            그리고 tbl_user의 id와 tbl_post의 user_id가 일치하는 경우
*/
...
/*
    ...
    3. 최종적으로 tbl_child 테이블에서 MIDDLE SELECT의 결과에 포함된 id를 가진 행을 선택
    2. MIDDLE SELECT문 (중간 서브쿼리):
                tbl_apply 테이블에서 field_trip_id 가 INNERMOST SELECT 문에 포함된 경우 child_id선택
    1. INNERMOST SELECT문 (가장 안쪽 서브쿼리):
                 tbl_field_trip 테이블에서 '매미'를 포함한 제목을 가진 행들의 id를 선택.
*/
select * from tbl_child
where id in
